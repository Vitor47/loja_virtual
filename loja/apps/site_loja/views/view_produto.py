...
def ofertas(request):
    if request.method == "GET":
        list_products = Produto.objects.filter(status=True, tipo_id=1).exclude(quantidade=0).order_by('-id')
        list_categorys = ProdutoCategoria.objects.all().order_by('-id')
...
def novidades(request):
    if request.method == "GET":
        list_products = Produto.objects.filter(status=True, tipo_id=2).exclude(quantidade=0).order_by('-id')
        list_categorys = ProdutoCategoria.objects.all().order_by('-id')
